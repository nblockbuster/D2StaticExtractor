...
#pragma endregion
	...
	if (BubbleHash != "")
	{
		...
		{
			if (getReferenceFromHash(pbhash, packagesPath) != "d6998080")
				continue;
			logger.Debug(pbhash);
		...
		if (!StaticLZs.size())
		{
			logger.Error("Static Loadzones Empty.");
			exit(22);
		}
...
bool ExportSingleLoadZone(std::string lzHash, std::string outputPath, bool bTextures, std::string texTypeIn)
{
		...
		while (m < (LUTS[LUTS[a].EntryC].EntryB + LUTS[LUTS[a].EntryC].EntryA))
		{
			...
			if (lodCulling)
			{
				...
				for (const auto &split : submesh->lodsplit)
				{
					...
					if (cursub->vertUV.size())
						newsub->vertUV = trimVertsData(cursub->vertUV, dsort, false);
					if (cursub->vertNorm.size())
						newsub->vertNorm = trimVertsData(cursub->vertNorm, dsort, false);
					if (cursub->vertCol.size())
						newsub->vertCol = trimVertsData(cursub->vertCol, dsort, true);
					if (cursub->vertColSlots.size())
						newsub->vertColSlots = cursub->vertColSlots;
	...
	for (auto &node : nodes)
	{
		master_map_empty->AddChild(node);
	} // applyMaterial(fbxModel, submesh, node); }
